    ...
    return (
        <div className='w-full mx-auto md:px-30 px-5 fixed top-10 z-30'>
            <div className="flex items-center justify-between">
                <div className="brand flex items-center gap-10">
                    <img src={theme === "light" ? '/portfoliologo.png' : '/llogo.png'} alt="Website Logo" className='h-15 transition-all cursor-pointer' onClick={()=> {
                        navigate('/');
                        setClicked(false);
                        }}/>
                    <h2 className={`otherFont text-2xl font-bold -translate-y-3 ${theme === "light" ? 'text-black' : 'text-white'} transition-all duration-300 opacity-0 ${clicked ? 'opacity-100 translate-y-0' : 'opacity-0'}`}>Kuljit Singh Dhaliwal</h2>
                </div>
